defmodule Explorer.Repo.Migrations.SetCoreContractForeignKeyOnEvents do
  ...
  def up do
    ...
    [block_numbers, indices] = from( cce in "celo_contract_events",
      left_join: ccc in "celo_core_contracts",
      on:  cce.contract_address_hash == ccc.address_hash,
      where:  is_nil(ccc.address_hash),
      select: [:block_number, :log_index])
    |> repo().all()
    |> Enum.reduce([[],[]], fn %{block_number: bn, log_index: li},[numbers, indices] -> [ [bn | numbers], [li | indices]] end)


    {deleted_row_count, _} = from(
      cce in "celo_contract_events",
      join: v in fragment("SELECT * FROM unnest(?::int[], ?::int[]) AS v(block_number,index)", ^block_numbers, ^indices),
    ...
    alter table("celo_contract_events") do
      modify :contract_address_hash, references("celo_core_contracts", column: :address_hash, type: :bytea, name: :contract_address_hash)
    end
  end
